...
' >> $output
    cat $input | grep -i 'Nmap scan report for \|printer\|print server\|lexmark\|canon\|brother\|xerox\|kyocera\|ricoh\|epson\|konica\|Virata-EmWeb\|jetdirect\|laserjet\|Service Info:' | sed 's/Nmap scan report for /\n/g' | grep -vi 'filtered\|closed\|#\|SF:\|SF-\|unrecognized\|incorrect\|Warning:\|Host is' | grep -B2 '/tcp' >> $output
}
